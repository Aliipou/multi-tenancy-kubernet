#!/bin/bash
# memory_pressure/run.sh — Measure II under a memory-pressure aggressor.
#
...
#
# Answers: RQ2 — Does LimitRange prevent memory-pressure cross-contamination?
#          RQ3 — What is the latency impact of kernel memory pressure?
...
deploy() {
  ...
  log "Deploying memory aggressor..."
  kubectl apply -f "$STRESS_MANIFEST"
  kubectl rollout status deployment/mem-aggressor -n "$NAMESPACE" --timeout=60s

  log "Letting memory pressure build for 15s..."
  sleep 15
}
...
run_loadtest() {
  ...
  log "Load test under memory pressure: ${DURATION}s @ concurrency=${CONCURRENCY}..."
  RAND_SEED=$SEED hey \
    -z "${DURATION}s" \
    -c "$CONCURRENCY" \
    -m GET \
...
compute_ii() {
  if [[ -n "$BASELINE_JSON" && -f "$BASELINE_JSON" ]]; then
    ...
    warn "No baseline JSON provided — II will be null."
    II="null"
  fi
}

capture_node_memory() {
  log "Capturing node memory stats..."
  kubectl top node 2>/dev/null > "$RESULTS_DIR/node-top.txt" || warn "metrics-server unavailable"
  kubectl top pods -n "$NAMESPACE" 2>/dev/null > "$RESULTS_DIR/pod-top.txt" || true
  # Check for OOMKilled events
  kubectl get events -n "$NAMESPACE" \
    --field-selector reason=OOMKilling 2>/dev/null \
    > "$RESULTS_DIR/oom-events.txt" || true
}
...
save_results() {
  ...
  local oom_count=0
  [[ -s "$RESULTS_DIR/oom-events.txt" ]] && oom_count=$(wc -l < "$RESULTS_DIR/oom-events.txt")

  jq -n \
    --arg exp       "memory_pressure" \
  ...
  '{
    ...
    workload: {
      target: "api-service /health (with mem-aggressor 512MB in same namespace)",
...
log "Memory pressure experiment complete."
